...
impl Frame {
    /// DUET PATCH: true when `acquire_frame` hit `VK_ERROR_OUT_OF_DATE_KHR`
    /// and handed back a placeholder that cannot be presented -- the caller
    /// must reconfigure the surface (recreate the swapchain) and acquire again.
