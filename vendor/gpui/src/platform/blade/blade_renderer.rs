...
impl BladeRenderer {
    ...
    pub fn draw(&mut self, scene: &Scene) {
        ...
        if frame.is_out_of_date() {
            // DUET PATCH: the swapchain is out of date (VK_ERROR_OUT_OF_DATE_KHR).
            // On Wayland this happens whenever the compositor changes the
            // surface's dmabuf feedback -- e.g. GNOME/Mutter offering direct
            // scanout once a maximized window covers the whole output --
            // and Mesa retires the swapchain until the client recreates it.
            // Without this, every subsequent acquire fails and the window
            // never presents again (it looks frozen while the process lives).
            log::warn!("surface out of date; recreating the swapchain at the current size");
            self.wait_for_gpu();
            self.gpu
                .reconfigure_surface(&mut self.surface, self.surface_config);
            frame = self.surface.acquire_frame();
            if frame.is_out_of_date() {
                log::error!("surface still out of date after recreating the swapchain");
            }
        }
