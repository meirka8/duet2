...
**ADR-007: Vendor-and-patch `gpui` and `blade-graphics` for the Wayland out-of-date swapchain freeze.** **Accepted (2026-09-04).** UAT on GNOME/Mutter found the window freezing (still visible, never repainting) some seconds after a maximize. Root cause, established from `WAYLAND_DEBUG` traces and a programmatic reproduction: once a maximized/fullscreen window covers the whole output, Mutter re-evaluates it for direct scanout and sends new `zwp_linux_dmabuf_feedback_v1` tranches; Mesa retires the Vulkan swapchain on any such change, and from then on every `vkAcquireNextImageKHR` returns `VK_ERROR_OUT_OF_DATE_KHR` until the client recreates it. `blade-graphics 0.7.1` hands gpui a placeholder frame in that case and `gpui 0.2.2`'s `BladeRenderer::draw` never checks for it or reconfigures, so the window never presents again. Neither upstream has a fix (0.2.2 is the latest published gpui; blade `main` still returns the placeholder). The fix is the ordinary Vulkan contract, recreate-and-reacquire, and is two hunks totalling ~20 lines, but it lives inside vendored code. Options weighed: (a) drop maximize/fullscreen from Duet's own chrome (shipped briefly as a stopgap, PR #58, superseded) — hides one trigger but not the defect, since any dmabuf-feedback change (un-maximize, fullscreen, a compositor scanout decision on a merely output-sized window) hits the same path; (b) an app-level workaround — impossible without the renderer's private state, and detecting the condition would mean scraping a dependency's log line; (c) fork-and-patch. (c) it is, kept as small as it can be: byte-for-byte copies of the two crates.io releases under `vendor/`, stripped of non-build files, with the patched hunks marked `DUET PATCH`, wired through `[patch.crates-io]` so no crate's declared version changes and ADR-002/R-G7's isolation is untouched. The cost is honest and bounded: every ADR-003 bump of either crate must re-apply or retire the hunks, and `vendor/README.md` carries the recipe plus a no-click verification run (`DUET_DEBUG_CHROME_CYCLE_MS`, a diagnostics-only hook in `duet-ui::workspace::run`) so the bump gate can prove the freeze stays fixed on the real compositor.
...
| ADR-007 | Vendor-and-patch `gpui`/`blade-graphics` for the Wayland out-of-date swapchain freeze | **Accepted (2026-09-04) — two small `DUET PATCH` hunks under `vendor/`, re-applied or retired at every ADR-003 bump** |
